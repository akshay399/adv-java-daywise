...
//Overriding form of the method CAN NOT add any NEW or BROADER CHECKED excs
	...
	public LoginServlet(String msg) {
		System.out.println("in parameterized constr of the " + getClass()); //this gives runtime error as the onnly const that will be called is default constructor, so if we do not have a default const in program it will give runtime error but if we have a default const then it will simply ignore the parameterizd const and use the default cinst
	}	
	
	public LoginServlet() {
		System.out.println("Default const " + getClass());
		System.out.println("config " + getServletConfig()); //null
	}
	public void init() throws ServletException/* ,SQLException */ {
		try {
			// get servlet config object from WC
			ServletConfig config = getServletConfig();//by the time we reach init() the default const will load all the configs from web.xml
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			else // => valid login --> proceed to role based authorization
			{
				...
				// Resp pkt : SC 302 | Location : admin + Set-Cookie : JSESSIONID:
